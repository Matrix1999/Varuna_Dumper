...
import top.niunaijun.varuna_dumper.core.IBActivityThread;
import top.niunaijun.varuna_dumper.core.VMCore;
import top.niunaijun.varuna_dumper.entity.AppConfig;
import top.niunaijun.varuna_dumper.core.IOCore;
import top.niunaijun.varuna_dumper.entity.dump.DumpResult;
import top.niunaijun.varuna_dumper.utils.FileUtils;
import top.niunaijun.varuna_dumper.utils.Slog;
...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    private synchronized void handleBindApplication(String packageName, String processName) {
        ...
        result.dir = new File(VarunaDumperCore.get().getDexDumpDir(), packageName).getAbsolutePath();
        try {
            PackageInfo packageInfo = VarunaDumperCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, BActivityThread.getUserId());
            if (packageInfo == null)
                return;
            ...
            Object boundApplication = ActivityThread.mBoundApplication.get(VarunaDumperCore.mainThread());
            ...
            // clear dump file
            FileUtils.deleteDir(new File(VarunaDumperCore.get().getDexDumpDir(), packageName));
            ...
            VarunaDumperCore.get().getAppLifecycleCallback().beforeCreateApplication(packageName, processName, packageContext);
            try {
            ...
            ActivityThread.mInitialApplication.set(VarunaDumperCore.mainThread(), mInitialApplication);
            ...
            e.printStackTrace();
            mAppConfig = null;
            VarunaDumperCore.getBDumpManager().noticeMonitor(result.dumpError(e.getMessage()));
            VarunaDumperCore.get().uninstallPackage(packageName);
        }
    }
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (!dir.exists() || dir.listFiles().length == 0) {
                    VarunaDumperCore.getBDumpManager().noticeMonitor(result.dumpError("not found dex file"));
                } else {
                    VarunaDumperCore.getBDumpManager().noticeMonitor(result.dumpSuccess());
                }
                VarunaDumperCore.get().uninstallPackage(packageName);
            }
        }).start();
    ...
    private Context createPackageContext(ApplicationInfo info) {
        try {
            return VarunaDumperCore.getContext().createPackageContext(info.packageName,
    ...
    public IBinder getActivityThread() {
        return ActivityThread.getApplicationThread.call(VarunaDumperCore.mainThread());
    }
