...
import top.niunaijun.varuna_dumper.utils.ArrayUtils;
...
public class PackageManagerCompat {
    ...
    public static PackageInfo generatePackageInfo(BPackage p, int flags, long firstInstallTime, long lastUpdateTime, BPackageUserState state, int userId) {
        ...
        PackageInfo pi = null;
        try {
            pi = VarunaDumperCore.getPackageManager().getPackageInfo(VarunaDumperCore.getHostPkg(), flags);
...
//                    }
    ...
    public static ApplicationInfo generateApplicationInfo(BPackage p, int flags, BPackageUserState state, int userId) {
        ...
        try {
            baseApplication = VarunaDumperCore.getPackageManager().getApplicationInfo(VarunaDumperCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        String sourceDir = p.baseCodePath;
        if (p.applicationInfo == null) {
            p.applicationInfo = VarunaDumperCore.getPackageManager()
...
//        ai.uid = p.mExtras.appId;
        ...
        if (BuildCompat.isL()) {
            ...
            if (supportedAbis != null) {
                boolean is64Bit = BuildCompat.isM() && Process.is64Bit();
                for (String abi : supportedAbis) {
                    if ((is64Bit && "arm64-v8a".equals(abi))
                            || (!is64Bit && "armeabi-v7a".equals(abi))) {
                        primaryCpuAbi = abi;
                        break;
                    }
                }
...
//            ApplicationInfo base = VarunaDumperCore.getContext().getApplicationInfo();
