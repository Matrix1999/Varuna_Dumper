...
import top.niunaijun.varuna_dumper.entity.pm.InstalledPackage;
import top.niunaijun.varuna_dumper.core.system.BProcessManager;
import top.niunaijun.varuna_dumper.core.system.ISystemService;
...
import top.niunaijun.varuna_dumper.utils.compat.PackageParserCompat;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, VarunaDumperCore.getHostPkg())) {
            try {
                return VarunaDumperCore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, VarunaDumperCore.getHostPkg())) {
            try {
                return VarunaDumperCore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = VarunaDumperCore.getContext().getContentResolver().openInputStream(Uri.parse(file));
            ...
            if (!support) {
                return result.installError(VarunaDumperCore.is64Bit() ? "not support armeabi-v7a abi" : "not support arm64-v8a abi");
            }
