...
import top.niunaijun.varuna_dumper.utils.DexUtils;
import top.niunaijun.varuna_dumper.utils.FileUtils;
import top.niunaijun.varuna_dumper.utils.compat.DexFileCompat;
...
public class VMCore {
    ...
    static {
        new File("");
        System.loadLibrary("varuna_dumper");
    }
    ...
    public static void cookieDumpDex(ClassLoader classLoader, String packageName) {
        ...
        File file = new File(VarunaDumperCore.get().getDexDumpDir(), packageName);
        ...
        VarunaDumperCore.getBDumpManager().noticeMonitor(result.dumpProcess(cookies.size(), atomicInteger.getAndIncrement()));
        for (int i = 0; i < cookies.size(); i++) {
            ...
            executorService.execute(() -> {
                cookieDumpDex(cookie, file.getAbsolutePath(), VarunaDumperCore.get().isFixCodeItem());
                VarunaDumperCore.getBDumpManager().noticeMonitor(result.dumpProcess(cookies.size(), atomicInteger.getAndIncrement()));
                countDownLatch.countDown();
            });
        }
